...
product-safe 现在挂载 Design Image Spike Agent。preset 目录是 `generate_image` 与 `edit_image`。确定性 mock PNG 生成器写入官方 Harness attachment。官方 `tool.call.toolview` 空洞渲染 1–4 个 variant。Continue Editing 写入显式 `[source:<attachmentId>]` token。刷新会话时，已完成图片从持久化 tool result 和 `session.attachment` 恢复。
...
M2 规格：19 个文件、83 个测试，全部通过。

全量套件：14665 通过。单独重跑那条超时的 oxlint-contract 已通过。其余 14 个失败是本环境缺少可选的 Codex/Claude real-product 二进制，不是 M2 图片工具回归。
