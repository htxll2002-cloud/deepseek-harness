...
- M2 new upstream runtime patches: 0
- Source token status: `TEMPORARY_M2_ONLY`
- Harness `attachmentId` is not a product `artifact_id`

## Gates

Typecheck, lint, and build passed.

M2 specs: 20 files, 84 tests, all passed.

Full suite: 14667 passed, 114 skipped. The 14 failures are optional Codex/Claude real-product binaries missing in this environment, not M2 image-tool regressions.

## Known gaps

- Selection is session-local spike state, not product artifact truth.
- `[source:<attachmentId>]` is M2 spike transport and must be replaced after product working state exists.
