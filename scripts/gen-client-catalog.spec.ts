...
describe('the real workspace surface', () => {
  it('collects every declared slot with a teachable contract', { timeout: 30_000 }, () => {
    ...
    const occupants = entries.flatMap(entry => entry.occupants).join('\n')
    expect(occupants).not.toContain('ui-design-image')
    const toolview = entries.find(entry => entry.key === 'tool.call.toolview')
    expect(toolview?.keyDomain).not.toContain('generate_image')
    expect(toolview?.keyDomain).not.toContain('edit_image')
  })
})
